...
public class BOJ15898 {
    final int SIZE = 3;
    final int MATTER_SIZE = 4;
    final int KILN_SIZE = 5;
    ...
    private void pickMatter(int l, Matter[][] kiln, int visited) {
        if (l == SIZE) {
    ...
    public Matter[][] rotatePick(Matter[][] pick) {
        Matter[][] tmp = makeKiln();
    ...
    private Matter[][] putMatter(Matter[][] tmp, Matter[][] pick, int y, int x) {
        for (int i = 0; i < MATTER_SIZE; i++) {
            for (int j = 0; j < MATTER_SIZE; j++) {
    ...
    private Matter[][] copyKiln(Matter[][] dst, Matter[][] src) {
        for (int i = 0; i < KILN_SIZE; i++) {
            for (int j = 0; j < KILN_SIZE; j++) {
    ...
    private int getQuality(Matter[][] kiln) {
        int r = 0, b = 0, g = 0, y = 0;
