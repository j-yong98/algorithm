...
public class BOJ5373 {
    ...
    private void turn(char side, char dir) {
        Cube tmp = new Cube();
    ...
    private char getSide(char side) {
        if (side == 'U')
            return 'D';
        if (side == 'D')
            return 'U';
    ...
    private void setCube(char side, Cube src, Cube dst) {
        if (side == 'U') {
            for (int i = 0; i < SIZE; i++) {
                for (int j = 0; j < SIZE; j++)
